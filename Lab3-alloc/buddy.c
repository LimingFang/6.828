...
#define LEAF_SIZE     16                         // The smallest block size, at least for struct list, i.e., 16B
...
bd_initfree_pair(int k, int bi, int left) {
  ...
  if(bit_isset(bd_sizes[k].alloc, bi/2)) {
    ...
    free = BLK_SIZE(k);
    // put which buddy on free list
    if (left)
      lst_push(&bd_sizes[k].free, addr(k, bi));
    else
      lst_push(&bd_sizes[k].free, addr(k, (bi/2)*2));
  }
  return free;
}
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char)* ROUNDUP((NBLK(k)+1)/2, 8)/8;
    bd_sizes[k].alloc = p;
